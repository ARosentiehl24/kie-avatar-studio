"""Tests del método `KieClient.create_kling_video_task` (Kling 3.0 video)."""
...
async def test_create_kling_video_task_posts_expected_body(
    ...
    result = await client.create_kling_video_task(
...
async def test_create_kling_video_task_accepts_custom_duration(
    mock_kie_client: tuple[KieClient, list[httpx.Request]],
) -> None:
    client, captured = mock_kie_client
    captured.clear()
    ...
    await client.create_kling_video_task(
...
async def test_create_kling_video_task_accepts_sound_true(
    mock_kie_client: tuple[KieClient, list[httpx.Request]],
) -> None:
...
async def test_create_kling_video_task_accepts_custom_mode_and_aspect(
    mock_kie_client: tuple[KieClient, list[httpx.Request]],
) -> None:
    client, captured = mock_kie_client
    captured.clear()
...
async def test_create_kling_video_task_accepts_custom_model(
    mock_kie_client: tuple[KieClient, list[httpx.Request]],
) -> None:
...
async def test_create_kling_video_task_propagates_4xx_as_client_error(
    mock_kie_client: tuple[KieClient, list[httpx.Request]],
) -> None:
