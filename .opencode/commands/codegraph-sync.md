---
description:
  Sincroniza el índice local de CodeGraph
  (.codegraph/codegraph.db). Pasá "full" para forzar reindex completo en
  lugar del sync incremental.
agent: build
---
...
- Argumento recibido: `$ARGUMENTS` (vacío ⇒ sync incremental, `full` ⇒ reindex
  completo).
- ¿CodeGraph instalado?:
  !`command -v codegraph >/dev/null && codegraph --version || echo "NO INSTALADO"`
- ¿Existe `.codegraph/codegraph.db`?:
  !`test -f .codegraph/codegraph.db && echo "sí ($(stat -c%s .codegraph/codegraph.db 2>/dev/null || stat -f%z .codegraph/codegraph.db) bytes)" || echo "NO"`
...
1. **Si CodeGraph no está instalado** (línea de arriba dice "NO INSTALADO"):
   parar y avisarme con el comando exacto: `pnpm i -g @colbymchenry/codegraph`
   (o `npm i -g @colbymchenry/codegraph`). No intentes nada más.
...
3. **Si el argumento fue `full`**: ejecutar `codegraph index` (reindex completo,
   reescribe toda la DB). Reservado para tras renames masivos o upgrade de
   versión.
